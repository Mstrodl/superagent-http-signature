...
var Authorization = 'keyId="%s",algorithm="%s",headers="%s",signature="%s"';
...
function sign(request, options) {
  ...
  request.set('Signature', `keyId="${options.keyId}",algorithm="${options.algorithm}",headers="${options.headers.join(' ')}",signature="${signature}"`)
  return true;
};
